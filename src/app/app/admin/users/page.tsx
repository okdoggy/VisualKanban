      ...
      <Card className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="w-full max-w-sm">
            <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름/아이디/권한 검색" />
          </div>
        </div>
          ...
          <table className="min-w-full border-separate border-spacing-0 text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                <th className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">Name</th>
                <th className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">Username</th>
                <th className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">Role</th>
                <th className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">Password State</th>
              </tr>
            </thead>
            <tbody>
              {visibleUsers.map((user) => (
                <tr key={user.id} className="odd:bg-zinc-50/70 dark:odd:bg-zinc-900/40">
                  <td className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">{user.displayName}</td>
                  <td className="border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">{user.username}</td>
